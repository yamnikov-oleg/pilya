...
synPrintStatement indent (Syn.StmtCondition expr thenBranch maybeElseBranch) = do
    ...
    case maybeElseBranch of
        ...
        Nothing         -> return ()
synPrintStatement indent (Syn.StmtForLoop ident init target body) = do
    printIndented indent $ "StmtForLoop " ++ ident
...
synPrintStatement indent (Syn.StmtWhileLoop expr body) = do
    ...
    synPrintExpr (indent + 1) expr
    synPrintStatement (indent + 1) body
synPrintStatement indent (Syn.StmtRead idents) = do
    printIndented indent "StmtRead"
    forM_ idents $ printIndented (indent + 1)
synPrintStatement indent (Syn.StmtWrite exprs) = do
    printIndented indent "StmtWrite"
    forM_ exprs $ synPrintExpr (indent + 1)
